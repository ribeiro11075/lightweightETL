...
CONFIG_DIRECTORY_VARIABLE = 'UNDERSTUDY_CONFIG'
MANIFEST_KEY_VARIABLE = 'UNDERSTUDY_MANIFEST_KEY'
NOTIFY_URL_VARIABLE = 'UNDERSTUDY_NOTIFY_URL'
...
def _configDirectory(arguments: argparse.Namespace) -> Path:
    """--config, else $UNDERSTUDY_CONFIG, else ./configuration -- so the
    common case is a bare `understudy run`.
    """
...
def _toolVersion() -> str:
    ...
    try:
        return version('understudy-data')
...
def _writeManifest(path: Path, result: RunResult, jobsFile: DataJobsFile, arguments: argparse.Namespace, log: Log) -> None:
    ...
    manifest.update(tool={'name': 'understudy-data', 'version': _toolVersion()},
...
def _generatedHeading(command: str, source: str, target: str) -> List[str]:
    ...
    return [
        'Generated by `understudy {}` on {} from {}, loading into {}.'.format(command, datetime.date.today().isoformat(), source, target),
        '',
...
def _nextSteps(source: str, target: str, tables: Sequence[str], related: bool = False) -> List[str]:
    ...
    if source == target:
        return [
            ...
            '  understudy schema --database {0} --target {0} {1} --stage-suffix _masked_stage --apply'.format(source, tableArguments),
            '',
            ]

    return [
        'Create any target tables that do not exist yet with:',
        '  understudy schema --database {} --target {} {}{} --apply'.format(source, target, tableArguments, ' --related' if related else ''),
        'To refresh the copy later, empty it first, then run every job:',
        '  understudy clear --config <this directory> --yes && understudy run --config <this directory> --force',
        '',
        ]
...
def _commandSchema(arguments: argparse.Namespace, log: Log) -> int:
    ...
    if not arguments.apply:
        heading = [
            'Generated by `understudy schema` on {} from {} ({}), for {} ({}).'.format(
...
def _addHistoryArguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--history-table', default='understudy_history', help='the --history-database table (default: understudy_history)')
...
def _buildParser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog='understudy', description='Move, mask and subset data between databases, with jobs defined in YAML.')
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for name, default in (('memory_table', 'understudy_memory'), ('history_table', 'understudy_history')):
        if not hasattr(arguments, name):
