...
def test_postgresql_options_reach_libpq():
    ...
    with _connect('postgresql', options={'application_name': 'lightweight-etl-test', 'sslmode': 'disable'}) as database:
        assert database.query("SELECT current_setting('application_name')") == [('lightweight-etl-test',)]
        assert database.isEncrypted() is False
...
def test_oracle_options_reach_the_driver():
    ...
    with _connect('oracle', options={'program': 'lightweight-etl-test'}) as database:
        assert database.query("SELECT program FROM v$session WHERE sid = SYS_CONTEXT('USERENV', 'SID')") == [('lightweight-etl-test',)]
        assert database.isEncrypted() is False
