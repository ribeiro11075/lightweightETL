...
- [`scramble.yaml` — masking jobs](#scrambleyaml--masking-jobs)
- [Validation](#validation)


## Where configuration is found

The CLI looks for a directory holding `database.yaml` plus either `jobs.yaml` (for `run`) or `scramble.yaml` (for `scramble`), in this order:
...
## `scramble.yaml` — masking jobs

Masking rewrites a table **in place**. Run it only against copies.
...
`workers`, `cycleSleepSeconds`, `active`, `refresh`, `predecessors`, `preTargetAdhocQueries` and `postTargetAdhocQueries` mean what they do for data jobs. Scramble jobs have no `retries` — see [why](design.md#retries).
...
Each column gets the first treatment that applies, in the order listed. **A column mentioned nowhere is shuffled, not left alone** — list anything you need preserved in `identifierColumns`.

Known limitations of the current masking are listed in [design.md](design.md#masking), including that `randomColumns` fails on non-integer numeric columns.


## Validation

`lightweight-etl validate` checks everything above without connecting to anything: every field, every alias, every predecessor, and every transformer reference. Problems are reported all at once, as `ConfigurationError`, rather than one per run.

`lightweight-etl run --dry-run` adds the checks that need a connection.
