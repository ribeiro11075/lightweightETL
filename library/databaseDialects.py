...
class OracleDialect(DatabaseDialect):
    ...
    def _mergeUpdateInsert(self, targetAlias: str, sourceAlias: str, allColumns: List[str], primaryKeyColumns: List[str], nonPrimaryKeyColumns: List[str]) -> str:
        """The shared ON/WHEN MATCHED/WHEN NOT MATCHED tail of a MERGE statement.

        WHEN MATCHED is omitted entirely when there are no non-primary-key columns,
        since an empty UPDATE SET is invalid Oracle syntax -- a table of only
    ...
    def upsertQuery(self, table: str, allColumns: List[str], primaryKeyColumns: List[str], nonPrimaryKeyColumns: List[str]) -> str:
        ...
        mergeClause = self._mergeUpdateInsert('target', 'source', allColumns, primaryKeyColumns, nonPrimaryKeyColumns)
